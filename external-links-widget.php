...
Plugin URI: http://championofthesun.com
Description: Custom CHS external site icons
Author: Steve Burt
Author URI: http://championofthesun.com
Version: 0.1
*/

class wELI extends WP_Widget {

	const VERSION = '1.2';

	function __construct(){
		$options = array(
			'description' => 'A widget that displays the external site icons.',
			'name' => 'External Link Icons'
		);
		parent::__construct('wELI','',$options);
	...
	function admin_setup() {
		wp_enqueue_media();
		wp_enqueue_script( 'external-link-icons', plugins_url('script.js', __FILE__), array( 'jquery', 'media-upload', 'media-views' ), self::VERSION );
	}	
	...
	public function widget($args,$instance){
		...
		for($i=0;$i<$numB;$i++)
		{
			...
			echo '<div class="col-lg-6 col-md-6 col-sm-12"><a '.$fol.$tar.'href="'.esc_attr($$link).'" class="site-icon">';
			if (preg_match('/image/i',$$show)>0) {
				echo '<img class="img-fluid" src="//'.$imgsrc.'" alt="'.esc_attr($$alt).'">';
			}
...
add_action('widgets_init','register_wELI');

function register_wELI(){
	register_widget('wELI');
}
